...
| Article footer navigation | `#atlas-footer > .atlas-local-footer` generated by `article-page.js` |
| Site footer record | `.article-workspace + .article-site-footer` generated by `article-page.js` |
| Article end boundary | `.article-end-marker` generated by `article-page.js` |
...
| Global command bar | `.function-bar.article-function-bar` generated by `article-page.js` |
...
- Use `.atlas-eof-divider` exactly once after the final content and references to close the complete article. When an authored EOF divider is absent, `article-page.js` supplies `.article-end-marker` as the fallback; never show both.
...
- Keep the generated article-end marker between authored content and the afterword. It is the visual boundary between the record and discovery/navigation UI.
- Keep Related/Latest compact: two horizontal cards per row on desktop and one per row on narrow screens.
- Do not hand-author footer link groups or the global command bar inside individual articles.
